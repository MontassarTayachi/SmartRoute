...
async def _generate_delivery_reference(db: AsyncIOMotorDatabase) -> str:
    # Keep references short and unique while remaining readable in operations.
    for _ in range(10):
        reference = f"DLV-{datetime.utcnow().strftime('%Y%m%d')}-{uuid4().hex[:8].upper()}"
        exists = await db["deliveries"].find_one({"reference": reference}, {"_id": 1})
        if not exists:
            return reference

    raise HTTPException(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        detail="Impossible de générer une référence de livraison unique.",
    )
...
def _parse_date_filter(date_value: str) -> tuple[datetime, datetime]:
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Le paramètre date doit être au format YYYY-MM-DD.",
        )
    ...
    *,
    ...
    if status_filter:
        if status_filter not in ALLOWED_DELIVERY_STATUSES:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Statut de livraison invalide.",
            )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Erreur base de donnees lors de la lecture des livraisons.",
        )


async def create_delivery(db: AsyncIOMotorDatabase, payload: DeliveryCreate) -> dict:
    now = datetime.utcnow()
    reference = await _generate_delivery_reference(db)
    ...
    try:
        result = await db["deliveries"].insert_one(document)
        document["_id"] = result.inserted_id
        return _format_delivery(document)
    except DuplicateKeyError:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Une livraison avec cette reference existe deja.",
        )
    except PyMongoError:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Erreur base de donnees lors de la creation de la livraison.",
        )


async def get_delivery_by_id(db: AsyncIOMotorDatabase, delivery_id: str) -> dict:
    delivery_oid = _to_object_id(delivery_id, field_name="livraison")

    try:
        delivery = await db["deliveries"].find_one({"_id": delivery_oid})
    except PyMongoError:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Erreur base de donnees lors de la lecture de la livraison.",
        )
    ...
    try:
        if delivery.get("vehicle_id"):
            vehicle = await db["vehicles"].find_one({"_id": delivery["vehicle_id"]})
        if delivery.get("driver_id"):
            driver = await db["drivers"].find_one({"_id": delivery["driver_id"]})
    except PyMongoError:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Erreur base de donnees lors du chargement des relations.",
        )

    return _format_delivery(delivery, vehicle=vehicle, driver=driver)


async def update_delivery(db: AsyncIOMotorDatabase, delivery_id: str, payload: DeliveryUpdate) -> dict:
    delivery_oid = _to_object_id(delivery_id, field_name="livraison")
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Conflit de donnees sur la livraison.",
        )
    except PyMongoError:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Erreur base de donnees lors de la mise a jour de la livraison.",
        )
...
async def assign_delivery(
    ...
    *,
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Conflit lors de l'affectation de la livraison.",
        )
    except PyMongoError:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Erreur base de donnees lors de l'affectation.",
        )

    return _format_delivery(updated, vehicle=vehicle, driver=driver)


async def update_delivery_status(db: AsyncIOMotorDatabase, delivery_id: str, status_value: str) -> dict:
    delivery_oid = _to_object_id(delivery_id, field_name="livraison")
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Erreur base de donnees lors du changement de statut.",
        )
